...
TEST_F(BuildEnvelopeTest, JuniperUpdatePreservesEmptyUpdateNotification) {
    DataManipulation dm;
    juniper_gnmi::SubscribeResponse response;
    response.mutable_update()->set_timestamp(987654321);

    Json::Value telemetry;
    telemetry["extension"] = R"({"eom":true})";
    std::string encoded;
    ASSERT_TRUE(dm.JuniperUpdate(response, encoded, telemetry));

    const Json::Value root = ParseEnvelope(encoded);
    EXPECT_EQ(root["extension"].asString(), R"({"eom":true})");
    EXPECT_EQ(root["notification_timestamp"].asUInt64(), 987654321U);
    EXPECT_TRUE(root["sensor_path"].isArray());
    EXPECT_TRUE(root["sensor_path"].empty());
}

TEST_F(BuildEnvelopeTest, JuniperUpdateRejectsSyncResponseWithoutUpdate) {
    DataManipulation dm;
    juniper_gnmi::SubscribeResponse response;
